...
class SearchPagingSource(private val api : TMDBApiService , private val auth : String , private val query: String) : PagingSource<Int , SearchEntity.Result>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchEntity.Result> = try {
        ...
        delay(1000)
        val response = api.searchCollection(authorization = auth , query = query , page = currentPage).body()?.toDomain()
        LoadResult.Page(
